...
export function importData(
  ...
  progress.complete = {
    html: g.SpreadsheetApp.Dialog.getHtml({
...
function loadData() {
  ...
  do {
    ...
    if (frame.length >= SKY.PAGE_SIZE) {
      progress.status = `Loaded page ${page++} (${data.length - 1} rows)`;
      progress.value++;
      progress.max++;
    } else {
      complete = true;
...
function connectToSpreadsheet(numRows: number, numColumns: number) {
  progress.status = 'Connecting to spreadsheet…';
  progress.value++;
...
function formatColumnHeaders() {
  progress.status = 'Formatting column headers…';
  progress.value++;
...
function updateMetadata() {
  progress.status = 'Updating metadata…';
  progress.value++;
...
function getCompletionMessage() {
  switch (target) {
    case Target.Update:
      return `<code>${range.getA1Notation()}</code> has been updated with the latest data from "${
        list.name
      }" on Blackbaud.${
        range.getA1Notation() != prevRange.getA1Notation()
    ...
    case Target.Selection:
      return `<code>${range.getA1Notation()}</code> has been connected to "${
        list.name
      }" on Blackbaud and the data has been imported.${
        range.getA1Notation() != prevRange.getA1Notation()
